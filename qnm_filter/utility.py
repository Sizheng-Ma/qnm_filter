...
def parallel_compute_ftau(self, f_arr, tau_arr, num_cpu=-1, **kwargs):
    """Parallel computation of self.likelihood_vs_f_tau
    ...
    self : Network class instance
        An instance of a Network class that will have self.likelihood_vs_mass_spin computed.
    f_arr : array-like
        array of the values of frequencies to calculate the likelihood function for.
    tau_arr : array-like
    ...
    results = Parallel(num_cpu)(delayed(self.likelihood_vs_f_tau)(i, j, **kwargs) for i, j in flatten_array)
