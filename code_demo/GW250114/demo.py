...
reconstruction and a QNM-removed (filtered) version using qnm_filter.
...
def load_posterior_maxL(
    ...
    Notes
    -----
    - Some datasets store final mass/spin under *_non_evolved keys; we guard for this.
    - qnm_filter.Filter.mass_unit(mass) converts physical mass to geometric time.
    """
    import qnm_filter  # imported here so the module can be linted even if absent at parse-time
...
def antenna_factors_for_ifos(
    ...
    """
    Build bilby-style IFOs and compute plus/cross antenna patterns at geocenter time.
...
def build_surrogate_signal(ctx: MaxLContext, sampling_frequency: int = 4 * 4096) -> Tuple[np.ndarray, np.ndarray]:
    ...
    -------
    time : np.ndarray
        Time samples for the surrogate (seconds).
    h_complex : np.ndarray (complex)
        Complex strain h = h_plus - i h_cross (in mks units).
    """
    ...
    # Reference/minimum frequency must come from the config in the HDF5
    with h5py.File(POSTERIOR_H5, "r") as f:
...
def main() -> None:
    # ------------------------ Load posterior & data ------------------------ #
    ctx, real_data, noise_data = gwosc_setup(detectors=("H1", "L1"), psd_seconds=64)

    # Antenna responses on high-rate grids for plotting overlays
    ifo_map_hi = antenna_factors_for_ifos(
        ctx, ("H1", "L1"), sampling_frequency=16384, duration=4.0, start_time=-2.0
    )
    ...
    # Example modes: (l, m, n, polarization)
    ...
    # Real-event network (uses sky location)
    plotting_cfg = dict(
        model_list=model_list,
    ...
    # Surrogate network (no sky needed; already projected)
    nrsur_cfg = dict(
        model_list=model_list,
        t_init=-0.1,
        segment_length=0.4,
